...
class KmpSqliteDriver(
  ...
  private val statements = Cache.Builder<Int, KmpStatement>()
  ...
  private fun <T> execute(
    identifier: Int?,
    createStatement: () -> KmpStatement,
    binders: (SqlPreparedStatement.() -> Unit)?,
    result: KmpStatement.() -> T,
  ): QueryResult.Value<T> {
    var statement: KmpStatement? = null
    if (identifier != null) {
  ...
  override fun execute(
    ...
    execute(identifier, { KmpPreparedStatement(connection.prepare(sql)) }, binders, { execute() })
  ...
  override fun <R> executeQuery(
    ...
    execute(identifier, { KmpQuery(sql, connection, parameters) }, binders) { executeQuery(mapper) }
...
internal interface KmpStatement : SqlPreparedStatement {
  fun execute(): Long
...
private class KmpPreparedStatement(
...
private class KmpQuery(
  ...
  override fun <R> executeQuery(mapper: (SqlCursor) -> QueryResult<R>): R {
    ...
    return try {
      mapper(KmpCursor(statement)).value
    } finally {
      statement.close()
    }
  }
