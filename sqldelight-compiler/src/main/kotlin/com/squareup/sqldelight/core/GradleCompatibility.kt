  ...
  fun validate(propertiesFile: SqlDelightPropertiesFile): CompatibilityReport {
    val (minimumIdeVersion, currentGradleVersion) = try {
      ...
      return CompatibilityReport.Incompatible(
        reason = "Plugin 'SQLDelight' is incompatible with the current version of the SQLDelight Gradle plugin. Upgrade the version of com.squareup.sqldelight:gradle-plugin.",
      )
    }
    ...
    if (currentGradleVersion < minimumGradleVersion) {
      return CompatibilityReport.Incompatible(
        reason = "Gradle plugin 'com.squareup.sqldelight:gradle-plugin' is incompatible with the current SQLDelight IntelliJ Plugin version. Use version $minimumGradleVersion or later."
      )
    }
