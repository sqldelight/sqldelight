...
class SelectQueryFunctionTest {
  ...
  @Test fun `projection with more columns than there are runtime Function types`() {
    ...
    assertThat(generator.customResultTypeFunction().toString()).isEqualTo("""
      |fun <T : kotlin.Any> select(mapper: (
      |    val1: kotlin.Long?,
      |    val2: kotlin.Long?,
      |    val3: kotlin.Long?,
      |    val4: kotlin.Long?,
      ...
      |    val29: kotlin.Long?,
      |    val30: kotlin.Long?
      |) -> T): com.squareup.sqldelight.Query<T> = com.squareup.sqldelight.Query(${query.id}, select, driver, ""${'"'}
      ||SELECT *
      ||FROM bigTable
      |""${'"'}.trimMargin()) { cursor ->
      |    mapper(
  ...
  @Test fun `adapted column in inner query exposed in projection`() {
    ...
    assertThat(generator.customResultTypeFunction().toString()).isEqualTo("""
      ...
      |) -> T): com.squareup.sqldelight.Query<T> = com.squareup.sqldelight.Query(${query.id}, someSelect, driver, ""${'"'}
      ||SELECT *
      ||FROM (
      ||  SELECT *, 1 AS ordering
      ||  FROM testA
