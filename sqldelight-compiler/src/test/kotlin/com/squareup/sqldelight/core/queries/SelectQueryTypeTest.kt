...
class SelectQueryTypeTest {
  ...
  @Test fun `nullable parameter not escaped`() {
    ...
    assertThat(generator.querySubtype().toString()).isEqualTo("""
      ...
      |    override fun createStatement(): com.squareup.sqldelight.db.SqlPreparedStatement {
      |        val statement = database.getConnection().prepareStatement(0, "SELECT * FROM socialFeedItem WHERE message IS NOT NULL AND userId ${"$"}{ if (userId == null) "IS" else "=" } ?1 ORDER BY datetime(creation_time) DESC", com.squareup.sqldelight.db.SqlPreparedStatement.Type.SELECT, 1)
